<template>
  ...
  >
    ...
    <div class="integral">{{this.integral}}分</div>
    <div class="game_over" v-if="gameOver">
      <p>哦豁，玩没了,那今天记得要长{{this.integral}}斤肉哦</p>
      <div @click="closeGameOver">结束游戏</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    mobileRule() {
      ...
      switch (
        this.nextMoveDirection // 移动方向，计算移动后的横纵坐标
      ) {
        case 0:
          y--;
          break;
    ...
    whetherEatFood() {
      // 是否吃到食物判断
      let state =
        JSON.stringify(this.snakeHead) === JSON.stringify(this.foodCoordinate); // 判断食物和蛇头是否在一个坐标上
      if (state) {
        // 吃到了
    ...
    generateSnakeTail() {
      ...
      // 取上下左右code最小的是新蛇尾
      let top =
        y - 1 >= 0 && this.structureArray[y - 1][x].state
    ...
    deathRule() {
      ...
      switch (
        this.nextMoveDirection // 移动方向(碰壁判断和碰蛇身判断)
      ) {
        case 0:
